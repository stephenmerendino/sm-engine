...
static_string_t sm::init_static_string(sm::memory::arena_t* arena, size_t size)
{
    static_string_t str;
...
wchar_t* sm::to_wchar_string(sm::memory::arena_t* arena, const static_string_t& s)
{
    return to_wchar_string(arena, s.c_str.data);
}

wchar_t* sm::to_wchar_string(sm::memory::arena_t* arena, const char* s)
{
	size_t len = strlen(s);
	size_t num_chars_converted = 0;
    wchar_t* unicode_str = (wchar_t*)sm::memory::alloc_array_zero(arena, wchar_t, len + 1);
